...
This project is a high-performance bridge between AI coding agents and Java codebases, including Gemini CLI, Claude Code, Claude Desktop, Codex, and other MCP-compatible tools. It functions as both a **Model Context Protocol (MCP) server** and a **Gemini CLI Extension**, providing professional-grade Java language intelligence through Eclipse JDT.LS.
...
### For Claude Code / Codex / Other MCP Clients

For Claude Code, Claude Desktop, Codex, or other MCP clients, you'll need to configure the server explicitly as an MCP tool.
...
*Note: When using Claude Code, Codex, or other MCP clients, the project-level `.env` file logic (like `JAVA_PROJECT=true`) might not be loaded natively unless supported by the specific MCP client implementation. You may need to manage project-specific variables within the client's configuration directly.*
...
### 🤖 Usage in Claude Code / Codex / Other MCP Clients

While Claude Code and Codex don't support Gemini's extension packaging, they can still leverage these skills through their project rule files:

1.  **Project-Level Rules**: Copy the contents (or relevant parts) of the `SKILL.md` files from the `skills/` directory into a `.clauderules`, `CLAUDE.md`, or `AGENTS.md` file in your Java project's root.
2.  **Global Rules**: Alternatively, you can add these instructions to your global Claude Code or Codex configuration.

By placing these rules in project rule files, Claude Code or Codex can automatically adopt these "expert workflows" whenever it detects it is working within this project environment.
